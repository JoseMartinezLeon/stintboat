...
# This other function is expected to call this callback function in its definition. 
	pub_odom = rospy.Publisher('/stintboat/odom', Odometry, queue_size = 10)
    ...
    # ModelState() - Initializes a new instance of the ModelState class
        state_msg.model_name = 'stintboat'
	...
	if abs(angle_to_goal - yaw)>0.1:
		...
		print ('Stintboat faces the waypoint')

waypoint_x = 5.0
waypoint_y = 5.0
rospy.init_node('odom_subscriber')
sub_odom = rospy.Subscriber('/stintboat/odom', Odometry, callback)
rospy.spin()
