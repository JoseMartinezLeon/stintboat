...
def adjust_sail(wind):
	...
	sail_pub = rospy.Publisher('/sailboat/sail_controller/command', Float64, queue_size=10)
    # pub = rospy.Publisher('topic_name', std_msgs.msg.String, queue_size=10)
	sail_pub.publish(position_msg.data)
	# Create the publisher to the rudder controller topic.
	rudder_pub = rospy.Publisher('/sailboat/rudder_controller/command', Float64, queue_size=10)
    # pub = rospy.Publisher('topic_name', std_msgs.msg.String, queue_size=10)
...
def listener_new():
	...
	rospy.Subscriber('/sailboat_description_wind_topic/direction', Float64, adjust_sail)
	rospy.spin()
